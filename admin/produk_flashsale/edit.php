...
<!-- Input Flash Sale -->
<div class="container">
    <h3 class="text-center mt-3 mb-5">Edit Produk Flash Sale</h3>
    <div class="card p-5 mb-5">
        ...
        <form method="POST" action="proses_edit.php?id=<?php echo $result['id_produk_flashsale'] ?>">
        <div class="form-group">
                ...
                <select class="form-control" name="id_bunga" id="id_bunga" required>
                    <?php foreach ($produk_bunga as $row): ?>
                        <option 
                            value="<?php echo $row['id_bunga'] ?>" 
                            <?php echo $row['id_bunga'] === $result['id_bunga'] ? 'selected' : '' ?>>
                                <?php echo $row['nama_bunga'] ?>
                        </option>
                    <?php endforeach ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select class="form-control" name="id_flash_sale" id="id_flash_sale" required>
                    ...
                    ?>
                        <option 
                            value="<?php echo $row['id_flash_sale'] ?>"
                            <?php echo $row['id_flash_sale'] === $result['id_flash_sale'] ? 'selected' : '' ?>>
                                <?php echo $flash_start.' s/d '.$flash_end ?>
                        </option>
                    <?php endforeach ?>
                </select>
            </div>
